...
% %---------------femaleVIVO------------
    ...
    for i_pic=13:length(dir_pic)
        ...
        % Set up the interactive figure with two subplots
        fprintf('Processing image: %s\n', dir_pic(i_pic).name);
        figure('Name', 'Interactive XYZ Viewer');
        
        % First subplot for the image without the card
        subplot(1, 2, 1);
        imshow(XYZ_cropped./200); % Display the cropped XYZ image normalized by 200
        title('Image without Card');
        
        % Second subplot for the image with the card
        subplot(1, 2, 2);
        imshow(XYZ1_cropped./200);
        title('Image with Card');
        
        fprintf('Click on either image to get XYZ values. Press Enter to move to the next pair of images.\n');
        ...
        while true
            ...
            % Get the current axes handle to determine which subplot was clicked
            current_axes = gca;
            
            x_int = round(x);
            y_int = round(y);
            
            if current_axes.Tag == 'XYZ_cropped_subplot'
                % Clicks on the first subplot
                if x_int > 0 && y_int > 0 && x_int <= size(XYZ_cropped,2) && y_int <= size(XYZ_cropped,1)
                    pixel_values = squeeze(XYZ_cropped(y_int, x_int, :));
                    fprintf('Clicked on the "without card" image at (x=%d, y=%d). XYZ values: [%.4f, %.4f, %.4f]\n', ...
                        x_int, y_int, pixel_values(1), pixel_values(2), pixel_values(3));
                else
                    fprintf('Clicked outside the bounds of the "without card" image.\n');
                end
            elseif current_axes.Tag == 'XYZ1_cropped_subplot'
                % Clicks on the second subplot
                if x_int > 0 && y_int > 0 && x_int <= size(XYZ1_cropped,2) && y_int <= size(XYZ1_cropped,1)
                    pixel_values = squeeze(XYZ1_cropped(y_int, x_int, :));
                    fprintf('Clicked on the "with card" image at (x=%d, y=%d). XYZ values: [%.4f, %.4f, %.4f]\n', ...
                        x_int, y_int, pixel_values(1), pixel_values(2), pixel_values(3));
                else
                    fprintf('Clicked outside the bounds of the "with card" image.\n');
                end
            else
                % A click occurred but not within a recognized subplot
                 fprintf('Click recognized, but not on either image. Please click on one of the displayed images.\n');
            end
        end
