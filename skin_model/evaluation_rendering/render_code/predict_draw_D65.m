...
n_colors=3;
hue_val = linspace(0, 1, n_colors + 1);
hue_val = hue_val(1:end-1); 
hsv_matrix = [hue_val', 0.8 * ones(n_colors, 1), 0.8 * ones(n_colors, 1)];
...
% 确保输出目录存在
output_dir = fullfile('..\..\data', 'correlation_results',"fullpara",Dtype,"CAT_interp",rs_type);
...
for i_model=1:length(new_names)
    ...
    for attribute_idx_in_list = 1:length(attributes_to_process)
        ...
        model_draw_folder = fullfile(fdQst_draw_folder, lastPart,attribute_serial);
        ...
        strcat("fitRes.mat")); 
        data_fitRes=load(fitRes_file_base,"par_all");
        

        for i_par = 1:length(pcn)
        figure(i_par);hold on;
        % for i_par = [11]
            ...
            pars_GT=data_fitRes.par_all;
            par_GT=data_fitRes.par_all(i_par,:);
            xlim([min(pars_GT(:,4))-1,max(pars_GT(:,4))+10])
            ylim([min(pars_GT(:,5))-10,max(pars_GT(:,5))+10])
            ...
            [par_use_i] = calculate_par_from_ellipse(hue_angle, chroma, ...
            ...
            if CT>=5000
                ...
                par_use_i(4:5)=[a_interp,b_interp];
            end
            text(par_direct_scale(4), par_direct_scale( 5), ...
             num2str(i_par), 'Color', colors(1,:), 'FontSize', ...
             8, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
            text(par_use_i(4), par_use_i(5), ...
             num2str(i_par), 'Color', colors(2,:), 'FontSize', ...
             8, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
            text(par_GT( 4), par_GT( 5), ...
             num2str(i_par), 'Color', colors(3,:), 'FontSize', ...
             8, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
            ...
            % 保存图片
            save_file_name = fullfile(model_draw_folder, strcat(pcn(i_par), '.png'));
