...
obs_type="non_model";
rs_type="back_special";
% rs_type="direct_scale";
% rs_type="use_i";

% 确保输出目录存在
output_dir = fullfile('..\..\data', 'correlation_results',"fullpara","noCAT","CAT_interp",rs_type);
...
for i_model=1:length(new_names)
    ...
    for i_par = 1:length(pcn)
        ...
        for attribute_idx_in_list = 1:length(attributes_to_process)
            ...
            % labNscore_file=fullfile("D:\work\VIVOskinExpe\analyze\AnalyseResults_p_free" ,...
            %     "noCAT",scale_type_origin,lastPart,"model_group",attribute_serial,"labNscore", ...
            %     strcat("labNscore_group",lower(lastPart),lower(current_pcn_name),".mat")); % 修改这里，lastPart应该在lower之前
            labNscore_file=fullfile("D:\work\VIVOskinExpe\analyze\AnalyseResults_p_free" ,...
                "noCAT",scale_type_origin,lastPart,obs_type_used,attribute_serial,"labNscore", ...
                strcat("labNscore_group",lower(lastPart),lower(current_pcn_name),".mat")); % 修改这里，lastPart应该在lower之前
            ...
            XYZwpre=curr_struct.XYZw_pre_val;
            D_pre= calculateD(CT, 0, Dtype);
            if attribute==7&&ismember(i_par,[1,2,5,6])
                disp("d")
            end

            if iOr=='r'&& (strcmp(rs_type,"direct_scale")|| ...
                    strcmp(rs_type,"back_special")&&ismember(i_par,[9,11]))

                source_folder=fullfile("D:\work\VIVOskinExpe\analyze\AnalyseResults_p_free\" + ...
                ...
                XYZ_scene_aft = CAT16_D(XYZ_scene_scaled, ...
                    wd65,XYZwpre./XYZwpre(2).*wd65(2),  D_pre);  
                lab_scene_aft=xyz2lab(XYZ_scene_aft,"d65_64");
                lab_scene_aft=xyz2lab(XYZ_scene_scaled,"d65_64");
                par(4:5)=lab_scene_aft(2:3);
            ...
            if CT>=5000
                ...
                lab_bf=[average(i_par,1),par(4:5)];
                XYZ_bf=lab2xyz2(lab_bf,"d65_64");
                XYZ_aft = CAT16_D(XYZ_bf, wd65,XYZwpre./XYZwpre(2).*wd65(2),  D_pre);  
                lab_aft=xyz2lab(XYZ_aft,"d65_64");
                par(4:5)=lab_aft(2:3);
            else
                load("documents\noCAT\CT_interpolation.mat",'CT_interpolation_data');
                CT_values=CT_interpolation_data{i_nation, attribute}.CT_7;
                for i_CT=1:7
                    a_hue_angle=CT_interpolation_data{i_nation, attribute}.a_hue_angle_all_CT(i_CT,:);
                    hue_values(i_CT,1)=a_hue_angle(1).*average(i_par,1) + a_hue_angle(2);
                    a_CL=CT_interpolation_data{i_nation, attribute}.a_CL_all_CT(i_CT,:);
                    chroma_values(i_CT,1)=a_CL(1).*log(average(i_par,1)) + a_CL(2);
                end                
