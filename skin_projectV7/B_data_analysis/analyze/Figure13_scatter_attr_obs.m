...
nation_names=["Oriental","Caucasian","South Asian","African","all"];
% 定义人种对应的lastParts索引
...
%% 保存
if strcmp(Dtype,"efit_p_free")
    save_folder_name=fullfile("ellip_pic_p_free",scale_type);
elseif strcmp(Dtype,"efit_p")
    save_folder_name=fullfile("ellip_pic_p",scale_type);
end
...
for i_nation = 1:length(nations)
    ...
    axis equal;
    if i_nation==4
        min_lim=2;max_lim=16;
    else
        min_lim=8;max_lim=22;
    end
    xlim([min_lim, max_lim]);
    ylim([min_lim, max_lim]);
    x = linspace(min_lim, max_lim, 100);
    plot(x, x, 'k--');
    
    save_folder = fullfile(save_folder_name, Dtype,"attr", "comparison", iOr);
    ...
    exportgraphics(h1, fullfile(save_folder, ...
    ...
    for i_obs=1:length(obs_types)
        ...
        for attribute = attributes
            ...
            C=sqrt(lab_mean(2).^2+lab_mean(3).^2);
            h=atan2d(lab_mean(3),lab_mean(2));
            
            % 根据 obs_type 选择不同的 plot_style
            if strcmp(obs_type, "non_model")
                plot_style_current = 'o';
            elseif strcmp(obs_type, "model_group")
                plot_style_current = '^';
            end
            
            scatter(h, C, 30, plot_style_current,'filled', ...
                'MarkerFaceColor', colors(i_obs, :), 'MarkerEdgeColor', colors(i_obs,:));
    ...
    xlabel('{\ith}');
    ylabel('{\itC*}');
    title([nation_names(i_nation), ' C-h']);
...
% 图像拼接
concatenate_images1(fullfile(save_folder_name, Dtype,"attr", "comparison", iOr),4);
concatenate_images1(fullfile(save_folder_name, Dtype,"attr", "comparison", iOr, "C_h"),4);
