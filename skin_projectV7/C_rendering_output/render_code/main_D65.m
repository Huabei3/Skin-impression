...
%      - deltaE2000.m                （色差计算）
% 3. 外部数据：
%    - A_characterization文件夹：显示模型校准数据（.mat文件）
...
for i_model=1:length(new_names)
    ...
    % dir_XYZfile=dir(fullfile("D:\work\VIVOskinExpe\renderCode\XYZ",iOr,lastPart,"\*.mat"));
    % dir_XYZfile=dir(fullfile("..\original_image_XYZ",lastPart,"\*.mat"));
    dir_XYZfile=dir(fullfile("..\..\..\skin_projectV6_bigFile\original_image_XYZ",lastPart,"\*.mat"));
    ...
    % save_folder=fullfile('rendered',render_type,obs_type,"D65",lastPart);
    ...
    %-----render attribute_serial-------------
        ...
        for attribute = attributes
            ...
            absPath=fullfile(pwd,'..\..\B_data_analysis\lab_and_p', ...
                            lastPart,obs_type,attribute_serial, ...
                            'ellipPara');
            fit_center_data_file=fullfile(absPath,'fitRes.mat');
            % char(java.io.File(absPath).getCanonicalPath())
            if exist(fit_center_data_file,'file')
                [~,i_par]=ismember(files(i).name(1:end-4),pcn);
                % 加载椭圆拟合参数（用于属性评分模型）
                fit_center_data=load(fit_center_data_file);
                if size(fit_center_data.par_all,1)<i_par
                    continue
                end
                ...
                if ~isempty(target_ab)
                    ...
                    for i_dlabs_90=1:size(dlabs_90,1)
                        ...
                        [out_rendering,dest_lab,bull_nosd]=...
                            img_AddRender_simp_D65(img,bull,bull_nosd,render_type,delta_Lab, ...
                            XYZ,noFaceRGB_file,if_wei,if_2mask,CCT);    
                        % [out_rendering,dest_lab,bull_nosd]=...
                        ...
                        % strcat(files(i).name(1:end-4),'_',"D65", ...
