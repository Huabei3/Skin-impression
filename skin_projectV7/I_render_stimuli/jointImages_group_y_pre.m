...
for prefix = keys(prefix_groups)
    ...
    for i = 1:length(subgroup_files)
        ...
        if isempty(underscore_pos)
            ...
            for i_pre=1:size(y_pre,1)
                if strcmp(y_pre{i_pre,end},name)
                    % labels{i}=
                    break
                end
            end
    ...
    for i = 1:num_images
        ...
        img_with_text = insertText(img_with_text, text_position_number, labels{i}, ...
            'FontSize', font_size_number, 'TextColor', text_color_number, 'BoxOpacity', 0);
        resized_images{i} = img_with_text;
    end
    ...
    % 保存拼接后的图像
    output_file = fullfile(output_folder, strcat(prefix, '.jpg'));
