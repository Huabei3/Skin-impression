...
for i_model=1:length(new_names)
    ...
    files = dir(strcat(strrep(source_folder,"add",""),'\*.jpg'));  % 读取文件夹中的所有.jpg文件
    for i = 1:numel(files)
        files(i).name=char(strcat(ct(i),".jpg"));
    end
    ...
    elseif strcmp(iOr,'r')
        dir_XYZfile=dir(fullfile("XYZ\rs",strrep(lastPart,"add",""),"*.mat"));  
    end
    ...
    save_folder=fullfile('rendered\33_i_wei\LUT',Dtype,obs_type,'100',lastPart);
    ...
    %-----render attribute_serial-------------
        ...
        for attribute = attributes
            ...
            if exist(fit_center_data_file,'file')
                ...
                %--------算中心-------
                [CCT] = find_CCT(strcat(strrep(lastPart,"add",""),'_',files(i).name(1:end-4)));
                ...
                LUT=load(datai_file);
                XYZw_LUT=LUT.XYZw;
                backgroundGray_file=strcat("backgroundGray\i\" + ...
