...
load(fullfile("aveSkinByHand","i","aveLab4typesModels.mat"), ...
    "ave_lab_mat","lab_cell_all","picname_check_all");
...
for i_rs = 1:numel(files)
    ...
    renCen(i_rs,:)=ave_lab_mat{i_type,1}(ind_ct(i_rs,1),:);
    points_select{i_rs,1}=repmat(renCen(i_rs,:),length(num_points),1)+num_points;
    points_select{i_rs,1}(:,1)=repmat(average(i_rs,1),length(num_points),1);
    points_select{i_rs,2}=points_select{i_rs,1}(33,:);
    points_select{i_rs,3}=files(i_rs).name(1:end-4);        
    points_select{i_rs,4}=ct(ind_ct(i_rs,1));
