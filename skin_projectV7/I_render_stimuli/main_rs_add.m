...
for i_big=1:height(dE_cell_big)
    [~,i_model]=ismember(dE_cell_big{i_big,1}(1:3),new_names);
    if ismember(new_names(i_model),["f09","f10","m09","m10"])
        continue
    end
    slashes1=find(dE_cell_big{i_big,1}== '_');
    slashes2=find(dE_cell_big{i_big,1}== '[');
    ...
    save_folder=fullfile('rendered\rs\adjust_add',lastPart);
    ...
    load(fullfile("light_r\model_tcp",strcat(model,".mat")));
        ...
        i=str2num(dE_cell_big{i_big,1}(7:slashes1-1));    
    ...
    %--------先跑小图看问题--------
        ...
        %pre_draw
        % draw_folder=fullfile(strrep(save_folder,lastPart,""),"theo_dlab",num2str(i_type));
        % if ~exist(draw_folder, 'dir')
        %     mkdir(draw_folder);
        % end
        % h1=figure(1);
        % plot_render_points(dlabs);
        % shift2933=dlabs(29,:)-dlabs(33,:);
        % squeeze_factor{i_factor,1}=strcat(lastPart,files(i).name);        
        % squeeze_factor{i_factor,2}=CCT;
        % squeeze_factor{i_factor,3}=shift2933(1,3)./shift2933(1,2);
        % squeeze_factor{i_factor,4}=sqrt(shift2933(1,2).^2+shift2933(1,3).^2);
        % squeeze_factor{i_factor,5}=factor;
        % i_factor=i_factor+1;
        % exportgraphics(h1,fullfile(draw_folder, ...
        %     strcat(lastPart,files(i).name)),'Resolution',150);
        % title(fullfile(draw_folder,strcat(lastPart,files(i).name)));
        % clf;
    %     %render
            i_points=str2num(dE_cell_big{i_big,1}(slashes1+1:slashes2-1));    % 
    %         %--------算中心-------
    % 
            dlab=dlabs(i_points,:);
            ...
            search_name=strcat(files(i).name(1:end-4),'_',sprintf('%02d', i_points), '*.jpg');
            dir_img_file=dir(fullfile(save_folder,search_name));
            if ~isempty(dir_img_file)
                dest_folder=fullfile(strrep(save_folder,"adjust","bf_chan_adj"));
                if ~exist(dest_folder,"dir")
                    mkdir(dest_folder);
                end
                movefile(fullfile(save_folder,search_name), ...
                    fullfile(dest_folder,search_name));
            end
            ...
            disp([num2str(i_points),'/',num2str(endCenter),'of', ...
                num2str(i),'/',num2str(numel(files)),' ',lastPart,files(i).name,' begin']);
