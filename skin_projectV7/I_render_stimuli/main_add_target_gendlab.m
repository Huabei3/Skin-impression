...
target_score=0.9; % 目标评分值（0-1之间，转换为百分比）
if target_score==0.5
    score_type="abs";
else
    score_type="rela"; 
...
for i_model=1:length(new_names)
    ...
    %-----render attribute_serial-------------
        for attribute = attributes
            ...
            fit_center_data_file=fullfile('..\skin_projectV2\data\ellipse_para', ...
            strrep(lastPart,"add",""),obs_type_used,attribute_serial, ...
            ...
            if exist(fit_center_data_file,'file')
                ...
                if ~isempty(target_ab)
                    ...
                    for i_dlabs_90=1:size(dlabs_90,1)
                        ...
                        img_file=strcat(files(i).name(1:end-4), ...
                            '_',attribute_serial,pic_serial);
                        dir_img_file=dir(img_file);
                        dlab_theo{i_the,1}=strcat(strrep(lastPart,"add",""), ...
                            img_file);
...
output_folder=fullfile(strrep(save_folder,lastPart,""));
if ~exist(output_folder,"dir")
    mkdir(output_folder);
end
save(fullfile(output_folder,strcat('theo_dlab',target_score_str,'.mat')) ,"dlab_theo");
mean(cell2mat(dlab_theo(:,2)))
disp(fullfile(output_folder,strcat('theo_dlab',target_score_str,'.mat')));