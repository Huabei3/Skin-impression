...
% 设置输入目录和输出目录
save_folder="D:\work\VIVOskinExpe\renderCode\rendered\33_i_lL_C\maleVIVOi";
input_folder = fullfile(save_folder,"33"); % 替换为实际的输入目录路径
if ~exist(input_folder,"dir")
    mkdir(input_folder);
end
dir_pic=dir(fullfile(save_folder,"*.jpg"));
for i_pic=1:length(dir_pic)
    slashes=find(dir_pic(i_pic).name=='_');
    if strcmp(dir_pic(i_pic).name(slashes+1:slashes+2),'33')
        oldFile=fullfile(dir_pic(i_pic).folder,dir_pic(i_pic).name);
        newFile=fullfile(input_folder,dir_pic(i_pic).name);
...
for i = 1:num_images
    ...
    slashes=find(dir_pic(i).name=='_');
    picname_group{i,1}=dir_pic(i).name(1:slashes-1);
    % slashe1 = find(image_files(i).name=='[');
    % slashe2 = find(image_files(i).name==',');
    % slashe3 = find(image_files(i).name==']');    
    % dlabs(i,1) = str2double(image_files(i).name(slashe1+1:slashe2(1)-1));
    % dlabs(i,2) = str2double(image_files(i).name(slashe2(1)+1:slashe2(2)-1));
    % dlabs(i,3) = str2double(image_files(i).name(slashe2(2)+1:slashe3-1));
...
for i = 1:num_images
    ...
    % 在图片上插入其他文本
        img_with_text = insertText(img_with_text, text_position_labels,picname_group{i,1}, ...
