...
source_folder='dsp\m05\i';
slashe = find(source_folder=='\');
lastPart = source_folder(slashe(end-1)+1:slashe(end)-1);
lastPart=[lastPart,'i'];
%--------------
...
for i = 1:numel(files) 
        ...
        for i_xyz=1:length(dir_XYZ)
            if strcmpi(dir_XYZ(i_xyz).name(1:end-4),files(i).name(1:end-4))
        ...
        datafile = 'calibResults\data_ipv18_3.mat';
        LUT=load(datafile);
        XYZw_LUT=LUT.XYZw;
