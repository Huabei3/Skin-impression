...
new_names = [ "f01", "f02", "f03","m01","m02","m03",...
    ...
    "f09","f10","m09","m10"];
Dtype="full";
load("rendered\i\checkLabFromPics_CA_SA.mat","dE_cell_big");
...
for i_big=1:height(dE_cell_big)
    ...
    save_folder=fullfile('rendered\i\adjust_add',lastPart);
    ...
    num_points=[zeros(length(num_points),1),num_points];
        ...
        [~,i]=ismember(upper(dE_cell_big{i_big,1}(5:slashes1-1)),ct);
    ...
    %--------先跑小图看问题--------
        ...
        if i_type==4
            dlab_CATed=adjust_dlabs(dlab_CATed,0.53);
        end
