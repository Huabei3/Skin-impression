function lab_aft=CAT_lab2lab1(lab_bf,Dtype,CCT,direction,XYZw1,XYZw2)
    wd65_64 = [94.811, 100.00, 107.304];
    XYZw_pre = CCT2xyz(CCT);
    for i_input=1:size(lab_bf,1)    
        if max(XYZw1./XYZw1(2).*wd65_64(2)-wd65_64) <=0.1     
            XYZ_bf(i_input, :) = lab2xyz2(lab_bf(i_input, :), 'd65_64');
        else
            XYZ_bf(i_input, :) = lab2xyz2(lab_bf(i_input, :), 'user',XYZw1);
        end
        ...
        if max(XYZw2./XYZw1(2).*wd65_64(2)-wd65_64)   <=0.1 
            lab_aft(i_input, :) = xyz2lab(XYZ_aft(i_input, :), 'd65_64');
        else
            lab_aft(i_input, :) = xyz2lab(XYZ_aft(i_input, :), 'user',XYZw2);
        end
