function h = plot_ellipsoid(m, C, npts, axh)
% PLOT_ELLIPSOIDS plots 2-d and 3-d Gaussian distributions
%
% H = PLOT_ELLIPSOIDS(M, C) plots the distribution specified by 
...
%
%  
% PLOT_ELLIPSOIDS(M, C, NPTS) plots the ellipse or 
...
%
% PLOT_ELLIPSOIDS(M, C, NPTS, AX) adds the plot to the
...
%  figure; 
%  h1 = plot_ellipsoid([1 1], [1 0.5; 0.5 1]);
%  h2 = plot_ellipsoid([2 1.5], [1 -0.7; -0.7 1]);
%  h3 = plot_ellipsoid([0 0], [1 0; 0 1]);
%  set(h2,'color','r'); 
%  set(h3,'color','g');
% 
...
%  figure;
%  h1 = plot_ellipsoid([1 1  0], [1 0.5 0.2; 0.5 1 0.4; 0.2 0.4 1]);
%  h2 = plot_ellipsoid([1.5 1 .5], [1 -0.7 0.6; -0.7 1 0; 0.6 0 1]);
%  h3 = plot_ellipsoid([1 2 2], [0.5 0 0; 0 0.5 0; 0 0 0.5]);
...
switch numel(m)
   case 2, h=show2d(m(:),C,npts,axh);
   case 3, h=show3d(m(:),C,npts,axh);
   otherwise
...
%-----------------------------
function h = show2d(means, C, npts, axh)
...
ap = [x(:) y(:)]';
[v,d]=eig(C); 
d = sqrt(d); % convert variance to sd
bp = (v*d*ap) + repmat(means, 1, size(ap,2)); 
h = plot(bp(1,:), bp(2,:), '-', 'parent', axh);

%-----------------------------
function h = show3d(means, C, npts, axh)
