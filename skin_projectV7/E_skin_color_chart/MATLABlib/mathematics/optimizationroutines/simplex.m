...
if strcmp(kernel,'gauss_kernel') || strcmp(kernel,'gauss4_kernel') || strcmp(kernel,'RBF_kernel')
    k = 1;
    format = '  % 4d        % 4d     %e     %.4f        %.4f      %s \n';
    fprintf(1, '\n Iteration   Func-count    min f(x)    log(gamma)     log(h)    Procedure\n\n');
...
elseif strcmp(kernel,'gaussadd_kernel') 
    k = 3;
    C = progress('init','Fine tuning step using CSA results using simplex method...');
end
