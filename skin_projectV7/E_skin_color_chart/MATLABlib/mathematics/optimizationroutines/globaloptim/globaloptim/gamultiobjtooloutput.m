...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:08:18 $
...
switch flag
    ...
    case 'iter'
        ...
        if  javaMethodEDT('getChangedState',optimtoolGui) || modelCheck
            ...
            [unused1,optnew,unused2,errOpt] = readOptimHashTable(hashProblem, hashOptions);
            try 
                if ~isempty(errOpt)
                    error('globaloptim:gamultiobjtooloutput:invalidModel',errOpt);
                end
