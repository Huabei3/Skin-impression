...
%   $Revision: 1.1.6.5 $  $Date: 2010/11/08 02:22:21 $
...
gaoutput(FitnessFcn,options,state,currentState);
    ...
    function [xhybrid,fhybrid] = callHybridFunction
        ...
        if ~any(strcmpi(hfunc,{'fminsearch','fminunc','patternsearch'}))
            msg = sprintf('%s is not an unconstrained optimization solver',upper(hfunc));
            msg = [msg, sprintf('\n%s',' using unconstrained solver FMINUNC as hybrid function.')];
            warning('globaloptim:gaunc:constrainedHybridFcn',msg);
            hfunc = 'fminunc';
        end
