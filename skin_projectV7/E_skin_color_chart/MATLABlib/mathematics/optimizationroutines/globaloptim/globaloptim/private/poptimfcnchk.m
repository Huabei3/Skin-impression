...
%   $Revision: 1.1.6.4 $  $Date: 2010/11/01 19:36:58 $
...
if strcmpi(Vectorized,'off')  % The function is not vectorized. 
    ...
    if numel(y) ~=1
        error(message('globaloptim:poptimfcnchk:objfunCheck', 'Your objective function must return a scalar value.'));
    end
elseif strcmpi(Vectorized,'on') % If vectorized is 'on', Completepoll MUST be 'on' too
    ...
    if 2 ~=numel(f)
        error(message('globaloptim:poptimfcnchk:objfunCheck', [ 'When ''Vectorized'' is ''on'', your objective function must ', 'return a vector of length equal to number of input points.' ]));
    end
    y = f(1);
end
