function out=xyz2cam97u(xyzstim,Lstim, Lstimscot,xyzcond,Lcond, Lcondscot)
...
%
% xyzstim = xyz van de stimulus
% xyzcond = xyz van het conditioning field (wat je zag net voor de
% stimulus)
...
%Lcs=2.26.*Lc;
if isempty(Ls);
    for i=1:size(xyzstim,1);Ls(i)=2.26.*L(i).*(CCTa(xyzstim(i,:),10,1)./4000-0.4).^(1/3);end;Ls=Ls';
end
if isempty(Lcs);
    for i=1:size(xyzcond,1);Lcs(i)=2.26.*Lc(i).*(CCTa(xyzcond(i,:),10,1)./4000-0.4).^(1/3);end;Lcs=Lcs';
end
%La=L.^(2/3)./200;
%Las=(Ls./2.26).^(2/3)./200.*2.26;
La=Lc;
Las=Lcs;
...
%HMsQ=[rho, gamma, beta, rhoa, gamma, betaa, H', HCR', HCY', HCG', HCB', M, s, Aa, As, A, Q];
%HMsQ=[L, x, y, M, s, Aa, As, A, Q];
