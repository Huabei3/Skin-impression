function XYZ=jch2xyzcam02c(JCh,XYZw,La,Yb,surround,Did,MCAT)

% JCh2XYZCAM02: inverse model of CIECAM02 corrected for yellow-blue and
% purple problem
...
if nargin>4;
    if isnumeric(surround);
        F=surround(1);Nc=surround(2);c=surround(3);
    else
       if strcmp(surround,'average');F=1;c=0.69;Nc=1;s=1;end % average surround
       if strcmp(surround,'avg');F=1;c=0.69;Nc=1;s=1;end % average surround
       if strcmp(surround,'disp')||strcmp(surround,'display');c=0.69;Nc=1;F=0;s=1;end % for computer display colours
       if strcmp(surround,'dim');F=0.9;c=0.59;Nc=0.9;s=1;end % dim surround
       if strcmp(surround,'dark');F=0.8;c=0.525;Nc=0.8;s=1;end % dark surround
       if strcmp(surround,'T1');F=1;c=0.46;Nc=0.9;s=1;end % ISO 3664 T1 surround
    end
else
    F=1;c=0.69;Nc=1;s=1;  % ISO 3664 P1, average surround
end
...
if Did==1
    ...
    if isempty(Did);
        ...
        if Did <=1;D=abs(Did);else;D=F*(1-(1/3.6)*exp((-La-42)/92));if D>1;D=1;end;if D<0;D=0;end;end
    end    
end
k=1/(5*La+1);
...
MCAT02i=inv(MCAT02);
 ...
 if nargin==7;MCAT02=MCAT;MCAT02i=inv(MCAT);end
...
MHCi=MCAT02*inv(MH);
   ...
   RGBpw(RGBpw<0)=0;%solve purple problem! with CAT02 based CAT transform
...
RGBp=[Rp,Gp,Bp];
RGBp(RGBp<0)=0;%solve purple problem! with CAT02 based CAT transform
RGBc=(MHCi*RGBp')';
