function JCh=xyz2NISSCAM(XYZ,XYZw,La,Yb,surround)
...
abd=(Mc*LMSnlr')';%ab = color opponent space, d is normaisation constant
...
h=(180/pi)*atan(abh(:,2)./abh(:,1))%intermediate hue

rp=0.6581.*(cos((9.1-h)*pi/180)).^0.5390;rp=real(rp);rp(rp<=0)=0;
gp=0.9482.*(cos((167.0-h)*pi/180)).^2.9435;gp=real(gp);gp(gp<=0)=0;
yp=0.9041.*(cos((90.9-h)*pi/180)).^2.5251;yp=real(yp);yp(yp<=0)=0;
bp=0.7832.*(cos((268.4-h)*pi/180)).^0.2886;bp=real(bp);bp(bp<=0)=0;


a2p=rp-gp;
b2p=yp-bp;

hp=atan2(b2p,a2p)*180/pi;
