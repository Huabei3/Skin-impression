...
obs_type="non_model";
load(fullfile("..\..\C_rendering_output\render_code\documents",iOr,"render_data2.mat"),"render_map");
Keys = keys(render_map);

% 确保输出目录存在
output_dir = fullfile('..\tables');
...
load("..\..\C_rendering_output\render_code\aveLab_D65_Asian.mat","labC_HD65");
...
for i_model=1:length(new_names)
    ...
    for i_par = 1:length(pcn)
        ...
        for attribute_idx_in_list = 1:length(attributes_to_process)
            ...
            cd_folder = pwd;
            cd_folder = fileparts(cd_folder);
            cd_folder = fileparts(cd_folder);
            absPath=fullfile(cd_folder,'B_data_analysis\lab_and_p', ...
                lastPart,obs_type_used,attribute_serial, ...
                'labNscore');

            % char(java.io.File(absPath).getCanonicalPath())
            labNscore_file=fullfile(absPath , ...
                strcat("labNscore_group",lower(lastPart),lower(current_pcn_name),".mat")); % 修改这里，lastPart应该在lower之前
            

            curr_struct=render_map(strcat(lower(lastPart),lower(current_pcn_name)));
            ...
            model_fupara_file=fullfile("model_fullpara",obs_type);
            ...
            if ~isempty(y) && ~isempty(p_group) && numel(y) == numel(p_group) && numel(y) > 1
                ...
                fprintf('警告: 无法计算模特 %s, 光源 %s, 属性 %s 的相关性。原因：y或p_group为空或大小不匹配或数据不足。\n', ...
