...
VIVO_noCAT_folder="D:\work\VIVOskinExpe\analyze\AnalyseResults_p\noCAT\scaled\no_fit\resTable\original_unscaled";
% VIVO_noCAT_folder="D:\work\VIVOskinExpe\analyze\AnalyseResults_p\noCAT\scaled\resTable\original_unscaled";
VIVO_noCAT_file=fullfile(VIVO_noCAT_folder,"Peggy_VIVO_table.mat");
        ...
        for i_par = 1:length(pcn)
            ...
            for attribute_idx_in_list = 1:length(attributes_to_process)
                ...
                if any(idx)
                    ...
                    fprintf('警告: VIVO_table 中无匹配 (scene contains "%s", model=%s, observer=%s, attr=%s)\n', ...
                        current_pcn_name, current_model_name, obs_type_used, current_attribute_name);
                    % 回退到旧方法：从 labNscore_file 加载
                    ...
                    if exist(labNscore_file, 'file') == 2
                        labNscore_data=load(labNscore_file, "lab_group", "p_group", "picname_group");
                        lab_group=labNscore_data.lab_group;
                        p_group=labNscore_data.p_group;
                        picname_group=labNscore_data.picname_group;
                        fprintf('  [回退] 改用 labNscore_file 加载\n');
                    end
                end
                ...
                if strcmp(ablation_type, "no_L_depend")
                    ...
                    if strcmp(ablation_type,"no_nation")
                        i_nation_used=5;
                    else
                ...
                % ========== 消融实验：根据 ablation_type 处理 par(:,4:5) ==========
                if strcmp(ablation_type, "scene_types") && strcmp(iOr, 'r')
                    % ---- 完整模型：使用 scene_type 分组级别的 rela_incre ----
                    ...
                    if exist(rela_file, 'file')
                        ...
                        if ~isempty(i_scene_type) && isfield(rela_data, 'rela_incre')...
                                &&~strcmp(ablation_type, "no_scene_types")
                ...
                if strcmp(ablation_type,"full_CAT") && ~isempty(lab_group_noCAT)
                    ...
                    XYZ_ave_bf = lab2xyz2(lab_group_noCAT, 'd65_64');
                    XYZ_ave_aft = CAT16_D(XYZ_ave_bf,  XYZw_pre_scaled,wd65_64, D);
                    lab_group_aft = xyz2lab(XYZ_ave_aft, 'd65_64');
                    y = calculate_y(lab_group_aft(:,2), lab_group_aft(:,3), par); % 假设 calculate_y 函数已定义
                else              
