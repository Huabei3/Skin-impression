function [r_original, r_scaled, de_original, de_scaled, y_original, y_scaled,cen_scaled_lab] = calculate_scaled_params(par_original, points_data, scores_data, dims_OPPO, ref_cen_lab, fit_type, ref_scale_lab)
    % 输入参数说明：
    ...
    % ref_cen_lab: 参考中心lab坐标（如par_Peggy_OPPO(1,5:7)）
    % fit_type: 拟合类型字符串（如"ellipsoidfit4"/"ellipsoidfit3_1"）
    
    % 1. 提取原始中心并缩放
    cen_original_lab = par_original(1,5:7);
    xyz_cen_original = lab2xyz2(cen_original_lab, "d65_64");
    if nargin < 7 || isempty(ref_scale_lab)
    ...
    par_scaled(1,5:7) = cen_scaled_lab;
