...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:09:12 $
...
if gaAugParam.currentTolFun < stallTol && maxConstr <= options.TolCon
    ...
    if ~isempty(linCon) && linCon.linconCheck && ~feasibleLinearConstraints
        ...
        reasonToStop = [reasonToStop, sprintf('\n%s', ' and constraints violation is less than options.TolCon.')];
        exitFlag = 4;
    end
    if options.Verbosity > 0
        fprintf('%s\n',reasonToStop);
...
if ~isempty(infMessage) && ~isempty(strmatch('optimlib:optimfcnchk',infMessage))
    error('globaloptim:GAAUGCONVERGED:NaNFval','%s','Constraint function returned non-real value;can not continue.');
end
