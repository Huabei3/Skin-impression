...
%   $Revision: 1.1.6.7 $  $Date: 2010/11/08 02:22:18 $
...
if (rem(currentIter,options.PlotInterval) ~=0)
    return;
end
...
if (strcmp(flag,'init') || isempty(findobj(0,'Type','figure','name',fname))) && ...
    ...
    % Give pause and stop buttons in the figure
    % Determine who is the caller
    callStack = dbstack;
    caller = callStack(end).file(1:end-2);
    if ~strcmp(caller,'optimguiswitchyard')
        ...
        uicontrol('string','Stop','Position',[5 10 50 20],'callback',{@buttonStop,pObj});
         ...
         uicontrol('string','Stop','Position',[5 10 50 20],'callback',{@buttonStop,[]});
    end
