...
%   $Revision: 1.1.6.4 $  $Date: 2010/11/01 19:36:50 $
...
while i <= numberargs
    ...
    if ~isempty(arg)                      % [] is a valid options argument
        if ~isa(arg,'struct')
            error('globaloptim:PSOPTIMSET:invalidArgument','Expected argument %d to be a string parameter name or an options structure\ncreated with PSOPTIMSET.', i);
        end
