...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:08:34 $

if nargin < 2
  error('globaloptim:SAOPTIMGET:inputarg','Not enough input arguments.');
end
if nargin < 3
  default = [];
end
...
if ~isempty(options) && ~isa(options,'struct')
  error('globaloptim:SAOPTIMGET:firstargerror','First argument must be an options structure created with SAOPTIMSET.');
end
...
if isempty(j)               % if no matches
  error('globaloptim:SAOPTIMGET:invalidProperty',sprintf(['Unrecognized property name ''%s''.  ' ...
                 'See SAOPTIMSET for possibilities.'], name));
elseif length(j) > 1            % if more than one match
  ...
  if length(k) == 1
    ...
    msg = sprintf('%s).', msg);
    error('globaloptim:SAOPTIMGET:ambiguousProperty',msg);
  end
end
