function  STATS=MAOV1(X,alpha,displaylevel)
...
%
if nargin < 2;alpha=0.05;displaylevel=0;end
if nargin < 3 displaylevel=0;end
if nargout == 0; displaylevel = 1;end
displaylevel=abs(displaylevel);%always positive or zero!

STATSs = MAOV1_(X,alpha,0,displaylevel);%use large sample calculations
STATSl = MAOV1_(X,alpha,10^308,displaylevel);%use small sample calculations
fieldnamess=fieldnames(STATSs);
for i=1:numel(fieldnamess);STATS.(char(fieldnamess(i)))=STATSs.(char(fieldnamess(i)));end
fieldnamesl=fieldnames(STATSl);
for i=1:numel(fieldnamesl);STATS.(char(fieldnamesl(i)))=STATSl.(char(fieldnamesl(i)));end

function STATS = MAOV1_(X,alpha,fmin,displaylevel)
P = [];PP = [];PLH = [];PR = [];
if nargin < 2,
   alpha = 0.05;
   fmin = 25;%cutoff sample size for small or large sample problem
   displaylevel = 1;
end; 
if nargin < 3
    fmin = 25;
...
if f >= fmin;  %approximation to chi-square statistic (Bartlett)

   if displaylevel>0;disp(' '),disp(['Considered as a large sample problem:']),end% (n >= ',num2str(fmin),').']),end
   v = p*(g-1);
   ...
   if displaylevel>0;
       ...
       fprintf('-----------------------------------------------------------------------------------\n');
       fprintf('%5.i%11.i%13.i%14.4f%15.4f%12.i%13.4g\n',f,g,p,LW,X2,v,P);
       fprintf('-----------------------------------------------------------------------------------\n');
   ...
   STATS.numberofdata = f;
   STATS.numberofsamples = g;
   STATS.numberofsvariables = p;
   STATS.WilkesLambdaChi2.v = LW;
   STATS.WilkesLambdaChi2.Chi2 = X2;
   STATS.WilkesLambdaChi2.p = P;
   STATS.WilkesLambdaChi2.conclusion = ds;
else  %approximation to Rao's F-statistic

   if displaylevel>0;disp(' '),disp(['Considered as a small sample problem:']),end% (n <',num2str(fmin),').']),end
   if p == 2 | (g-1) == 2
      s = 2;
   else
   ...
   if displaylevel>0;
       ...
       fprintf('------------------------------------------------------------------------------\n');
       fprintf('Rao            %13.3f%10i%8i%11.2f %9.4g      %s\n',LW,v1,v2,F,P,ds1);
       fprintf('Pillai         %13.3f%10i%8i%11.2f %9.4g      %s\n',V,v1P,v2P,FP,PP,ds2);
       fprintf('Lawley-Hotelling      %6.3f%10i%8i%11.2f %9.4g      %s\n',U,v1LH,v2LH,FLH,PLH,ds3);
       fprintf('Roy            %13.3f%10.1f%8.1f%11.2f %9.4g      %s\n',R,v1R,v2R,FR,PR,ds4);
       fprintf('------------------------------------------------------------------------------\n');
   ...
   STATS.numberofsamples = g;
   STATS.numberofsvariables = p;
   STATS.WilkesLambdaRaoF.v = LW;
   STATS.WilkesLambdaRaoF.df1 = v1;
   STATS.WilkesLambdaRaoF.df2 = v2;
   STATS.WilkesLambdaRaoF.F = F;
   STATS.WilkesLambdaRaoF.p = P;
   STATS.WilkesLambdaRaoF.conclusion = ds1;
   STATS.Pillai.v = V;
   STATS.Pillai.df1 = v1P;
   STATS.Pillai.df2 = v2P;
   STATS.Pillai.F = FP;
   STATS.Pillai.p = PP;
   STATS.Pillai.conclusion = ds2;
   STATS.LawleyHotelling.v = U;
   STATS.LawleyHotelling.df1 = v1LH;
   STATS.LawleyHotelling.df2 = v2LH;
   STATS.LawleyHotelling.F = FLH;
   STATS.LawleyHotelling.p = PLH;
   STATS.LawleyHotelling.conclusion = ds3;
   STATS.Roy.v = R;
   STATS.Roy.df1 = v1R;
   STATS.Roy.df2 = v2R;
   STATS.Roy.F = FR;
   STATS.Roy.p = PR;
   STATS.Roy.conclusion = ds4;
end;
