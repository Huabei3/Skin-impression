function JCh=xyz2cam97s(XYZ,XYZw,LA,surroundc,adap,XYZb)
...
% SYNTAX
% ----------------------------------------------------------------------------
% JCh=CIECAM97(xyz,xyYW,xyYB,YA,VIEW,ADAP);
...
%
%
if nargin<6;XYZb=XYZw;end
if nargin<5;adap=2;end%normal adaptation

switch surroundc %switch to same conventions as ciecam02
    case 'average4'
        surr=1;
    case 'average'
        surr=2;
...
test=[xyz2xyY(XYZ),XYZ(:,2)];
white=[xyz2xyY(XYZw),XYZw(:,2)];
back=[xyz2xyY(XYZb),XYZb(:,2)];
...
%los pasos no lineales se hacen con un bucle:
  ...
  %a(i)=C1(i)-(C2(i)/11);
  a(i)=roa(i)-(12/11)*gaa(i)+bea(i)/11;
  b(i)=(1/9)*(roa(i)+gaa(i)-2*bea(i));
