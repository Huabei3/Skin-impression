...
function Ynew=sprague(X,Y,Xnew)
 f=abs(X(1)-X(2))/abs(Xnew(1)-Xnew(2));
 
 if (f<2 | ((f-floor(f))>0))
    disp('invalid sampleinterval value due to invalid f number - premature termination');
    disp(sprintf('\n f = %1.2f',f)),p = 0;Ynew=NaN;
    return;
 end

 Ynew=sprague_(Y',f)';
if Xnew(1)<X(1) 
    Ynew=[repmat(Ynew(1),abs(Xnew(1)-X(1))/(abs(Xnew(1)-Xnew(2))),1);Ynew];
end   
if Xnew(end)>X(end);
   Ynew=[Ynew;repmat(Ynew(end),abs(Xnew(end)-X(end))/(abs(Xnew(1)-Xnew(2))),1)];
end
if Xnew(1)>X(1) 
    r=abs(Xnew(1)-X(1))/(abs(Xnew(1)-Xnew(2)))+1;
    Ynew=[Ynew(r:end)];
end   
if Xnew(end)<X(end);
    r=abs(Xnew(end)-X(end))/(abs(Xnew(1)-Xnew(2)));
   Ynew=[Ynew(1:end-r)];
end
end

function [p] = sprague_(spectra,f)
% ===================================================
% *** FUNCTION sprague
% ***
