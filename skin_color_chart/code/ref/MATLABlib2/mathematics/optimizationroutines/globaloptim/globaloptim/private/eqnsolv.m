...
%   $Revision: 1.1.6.2 $  $Date: 2009/10/10 20:09:06 $
...
if issparse(Aeq)
    warning('globaloptim:EQNSOLV:sparseToFull','The equality constraint matrix is sparse; converting to a full matrix.');
    Aeq = full(Aeq);
    Beq = full(Beq);
end
if issparse(Aineq)
    warning('globaloptim:EQNSOLV:sparseToFull','The inequality constraint matrix is sparse; converting to a full matrix.');
    Aineq = full(Aineq);
    Bineq  = full(Bineq);
end
A = [Aeq;Aineq];
