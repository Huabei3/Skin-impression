...
%   $Revision: 1.1.6.5 $  $Date: 2010/11/08 02:22:17 $
...
while run
    ...
    if ~strcmp(gaAugParam.step,'Infeasible')
        if any(strcmp(subtype,{'boundconstraints','linearconstraints'}))
            [unused1,Iterate.f,innerExitFlag,innerOutput,innerPopulation,innerScore] = ...
...
% Hybrid function
    function [xhybrid,fhybrid] = callHybridFunction
        ...
        if ~any(strcmpi(hfunc,{'fmincon', 'patternsearch'}))
            msg = sprintf('%s is not a nonlinear constrained solver',upper(hfunc));
            msg = [msg, sprintf('\n%s',' using nonlinear constrained solver FMINCON as hybrid function.')];
            warning('globaloptim:gacon:unconstrainedHybridFcn',msg);
            hfunc = 'fmincon';
        end
