...
%   $Revision: 1.1.6.5 $  $Date: 2010/11/01 19:36:49 $
...
if ~isempty(errmsg)
    error('globaloptim:patternsearch:inputArg','%s PATTERNSEARCH requires at least 1 input argument.',errmsg);
end
...
try
    ...
    if ~isequal('double', dataType)
        error(message('globaloptim:patternsearch:dataType'))
    end
catch
    error(message('globaloptim:patternsearch:dataType'))
end
